...
published in the time span 1994–2016. The results of the review show that are tho types of workload models applied on stress tests generative models and descriptible models. Generative model just try to mimic the phenomena observed in the workload, whereas generative models try to emulate the process that generated the workload to fault-inducing. The Workload design phase of stress testing uses three different aproaches: model-based tests, search-based tests and FOREPOST. In model-based stress testing, tests use a model to describe the behavior of a system.Search-Based Software Testing is the use of a meta-heuristic optimizing search technique to automate or partially automate a testing task. Feedback-ORiEnted PerfOrmance Software Testing (FOREPOST) is an adaptive, feedback-directed learning testing system that learns rules from system execution traces. The load test tools was classified into three different groups using 12 common features. The research present a serie of 11 common performance problems.
\end{abstract}
...
\begin{minipage}{0.5\textwidth}
\centering
\includegraphics[width=1\textwidth]{./images/tpcw.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{./images/group3.png}
